properties {
  $Build_Solution = 'SqlToGraphitePlugin-LogParser.sln'
  $Debug = 'Debug'
  $pwd = pwd
  ...
  $version = '0.1.0.0'
  $nuspecFile = "SqlToGraphitePlugin-LogParser.nuspec"
}
...
task Package {
   if ((Test-path -path $Build_Artifacts -pathtype container) -eq $false)
	...
	Copy-item .\src\SqlToGraphite.Plugin.LogParser\output\SqlToGraphite.Plugin.LogParser.dll $Build_Artifacts\
...
task Init {
	...
	$Company = "peek.org.uk";
	$Description = "SqlToGraphite plugin for collecting metrics using LogParser";
	$Product = "SqlToGraphite LogParser Plugin $version";
	$Title = "SqlToGraphite LogParser Plugin $version";
